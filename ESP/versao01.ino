// Configurações do SQL
  ...
  IPAddress server(192, 168, 241, 226); //Trocar pelo IP da máquina PELO IP DA SUA BASE SQL
...
void runInsertPositivePresence(){
  ...
  if (conn.connected()){
    ...
    String sql_query = "insert into icaro.supino01 (status) values(1);";
...
void runInsertNegativePresence(){
  ...
  if (conn.connected()){
    // Comando SQL insert teste
    String sql_query = "insert into icaro.supino01 (status) values(0);";
    MYSQL_DISPLAY(sql_query);
